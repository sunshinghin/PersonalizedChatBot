...
SETUP_TEXT = "Hello! What I can do for you?"
...
def pngtuber_window_process():
    ...
    pygame.display.set_icon(appicon_image)
    ...
    while pngtuber_running:
        ...
        blit_text(surface=screen,dialog_box_size=(540,180),text=pngtuber_words_to_display,font=font,color=LIGHT_PINK,pos=(50,410))
...
class SettingsForm(FlaskForm):
    ...
    thinking_model_name_input = SelectField(label="ThinkingModelName",choices=[("gemma3:4b","gemma3:4b"),("qwen3:4b","qwen3:4b"),("deepseek-r1:7b","deepseek-r1:7b"),("llama3:8b","llama3:8b"),("llama3:70b","llama3:70b")],default="gemma3:4b",description="Select your thinking model name",render_kw={"class": "form-select"})
...
class MemoryResponse(BaseModel):
    ...
    is_conversation_important: bool
...
def index_page():
    ...
    if can_request and locks_for_prev_request.locked() == False:
        ...
        if transcripted_done == True:
            ...
            if chatform.FileUploadBox.data:
            ...
            with open("./memory.json", "r", encoding="utf-8") as f:
                ...
                try:
                    #load the json file back to a dict
                    prev_memory_log = json.loads(json_saved)
                except json.JSONDecodeError:
                    prev_memory_log = {}

            reverse_chat_log = dict(reversed(prev_memory_log.items()))
            ...
            action_response = chat(messages=[{

                "role": "user",
                "content": f"This is a coversation between two people.What actions you think needs to take to answer this question: {raw_input}? Only answer in these options: take computer screenshot,just chatting,use real life newspaper,use search result on google.",
            ...
            StructuredAction = ActionResponse.model_validate_json(action_response["message"]["content"])

            action = StructuredAction.action
            search_keyword = StructuredAction.search_keyword

            print("Action should take: " + action)
            print("search keyword: " + StructuredAction.search_keyword)
            ...
            if "google" in action.lower():

                num_results = 5

                search_result = search(search_keyword,num_results=num_results,advanced=True)

                search_num = 1

                for result in search_result:

                    search_results_text += "Title" + str(search_num) + ": " + result.title + ",Description" + str(search_num) + ": " + result.description + "."
            ...
            input_chat = f"""
                ...
                You are going to role play as a real person in a daily conversation.
                ...
                {"SUMMARY OF PREVIOUS CONVERSATION: " + Memory + "." if Memory != "" else ""}

                {"QUESTIONS AND ANSWERS IN CHAT HISTORY: " + asked_questions_memoery + "." if asked_questions_memoery != "" else ""}

                {"SEARCHING FROM GOOGLE: " + search_results_text + "." if search_results_text != "" else ""}

                {"THOUGHT ABOUT USER: " + thought_about_user + "." if thought_about_user != "" else ""}

                {"PERSONALITY: " + personality + "." if personality != "" else ""}

                {"DESCRIPTION OF IMAGES: " + images_desc + "." if images_desc != "" else ""}

                {"DOCUMENTS TEXT: " + document_desc + "." if document_desc != "" else ""}

                ANSWER THIS QUESTION IN UNDER 25 WORDS WITH NO EMOJI: {raw_input}.

                INSTRUCTION:

                You can change your mood and thought and face to make based on the question and answer.

                Return a website link when the user's action is needed to open a website.Otherwise,just return empty string.
            ...
            response = chat(messages=[{
                ...
                "role": "user",
                "content": f"Summarize the following conversation and stack with the Previous Summary and include the time: {time_now} in under 40 words and determine is this conversation has important details to memorize for future use.Previous Summary: {Memory}.Conversation: User: {raw_input},AI: {Structuredoutput.response_to_user}",
                
            }],
            

                model=thinking_model,
                format=MemoryResponse.model_json_schema(),

            )

            StructuredMemory = MemoryResponse.model_validate_json(response["message"]["content"])

            long_term_memory = StructuredMemory.conversation_summary

            is_conversation_important = StructuredMemory.is_conversation_important

            #end

            print("Summary memory: " + long_term_memory)

            print("is important: " + str(is_conversation_important))
            ...
            prev_chat_log[chatnumber] = {"Question": raw_input, "Answer": Structuredoutput.response_to_user, "Time": time_now}
            ...
            if is_conversation_important == True:

                write_memory_log = {}

                prev_memory_log[chatnumber] = {"Question": raw_input, "Answer": Structuredoutput.response_to_user, "Time": time_now}

                write_memory_log = prev_memory_log

                with open("./memory.json", "w",encoding="utf-8") as f:

                    json.dump(write_memory_log,f)
